...
void CX86Assembler::CmppsVo(XMMREGISTER registerId, const CAddress& address, SSE_CMP_TYPE condition)
{
	WriteEdVdOp_0F(0xC2, address, registerId);
	WriteByte(static_cast<uint8>(condition));
}
...
void CX86Assembler::CmpgtpsVo(XMMREGISTER registerId, const CAddress& address)
{
	CmppsVo(registerId, address, SSE_CMP_NLE);
}
...
void CX86Assembler::PaddsbVo(XMMREGISTER registerId, const CAddress& address)
{
	WriteEdVdOp_66_0F(0xEC, address, registerId);
}
...
void CX86Assembler::WriteEdVdOp_0F(uint8 opcode, const CAddress& address, XMMREGISTER xmmRegisterId)
{
	...
	WriteByte(opcode);
	NewAddress.Write(&m_tmpStream);
	WriteLiteralPlaceholder(address);
}
